...
-- ID, PARENT_ID, SIZE_OF_COLONY, DIFFERENTIATION_DATE, GENOTYPE 은 각각 대장균 개체의 ID, 부모 개체의 ID, 
...
-- 코드를 작성해주세요
select if(quarter(differentiation_date) = 1, '1Q', 
          if(quarter(differentiation_date) = 2, '2Q', 
            if(quarter(differentiation_date) = 3, '3Q', '4Q')
            )
          )as quarter, count(*) as ecoli_count from ecoli_data
group by quarter
order by quarter asc
-- if를 중첩하는것보다 concat으로 끝에 'Q'만 붙여주는게 효율
